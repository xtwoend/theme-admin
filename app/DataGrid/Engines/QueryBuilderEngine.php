...
use App\DataGrid\Helper;
use App\DataGrid\Request;

/**
 * Class QueryBuilderEngine.
 *
 * @package App\DataGrid\Engines
...
{   
     ...
     * @param \App\DataGrid\Request $request
     */
     ...
     * @param string $relation
     */
    protected function compileQuerySearch($query, $column, $keyword, $relation = 'or', $operator = 'LIKE')
    {
    ...
    public function columnSearch()
    {
        ...
        $groupOp = ($columns['groupOp'] == 'AND')? '': 'or';

        // edit search xtwoend
        foreach ($columns['rules'] as $index => $filter) {
            
            // $column = $this->getColumnName($index);
            $column = $filter['field'];
            $filter = $this->request->searchFilter($filter);

            // $this->operator = $filter['op'];
            ...
            if (isset($this->columnDef['filter'][$column])) {
                ...
                $this->compileColumnSearch($index, $column, $keyword, $filter['op'], $groupOp);
            }
     ...
     * @param string $keyword
     */
    protected function compileColumnSearch($i, $column, $keyword, $operator, $groupOp = 'OR')
    {   
        if ($this->request->isRegex($i)) {
            $column = strstr($column, '(') ? $this->connection->raw($column) : $column;
            $this->regexColumnSearch($column, $keyword);
        } else {
            $this->compileQuerySearch($this->query, $column, $keyword, $groupOp, $operator);
        }
    }
    ...
    {
        $start = ($this->request->input('rows') * $this->request->input('page')) - $this->request->input('rows');

        $this->query->skip((int) ($start > 0)? $start : 0)
                    ->take((int) $this->request->input('rows') > 0 ? $this->request->input('rows') : 10);
    }
