...
use App\DataGrid\Contracts\DataTableEngineContract;
use App\DataGrid\Helper;
use App\DataGrid\Processors\DataProcessor;
...
abstract class BaseEngine implements DataTableEngineContract
{
    ...
    {
        $count = $this->filteredRecords;
        $totalPages = ($count > 0)? ceil($count/$this->request->input('rows')) : 0;
        ...
        if (isset($this->transformer)) {
            ...
            $output['rows'] = $collection['data'];
        } else {
            $output['rows'] = Helper::transform($this->getProcessedData($object));
        }
